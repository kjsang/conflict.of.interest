...
data_prep_ver2 %>% 
  ...
  filter(title %>% str_detect("김영란|이해충돌|이해관계|공무원|청탁|공직|뇌물|부패|비리|공정|LH|특권|관피아")) -> data_prep_ver3 # 584 건
...
data_prep_ver4 %>% filter(title %>% str_detect("파워인터뷰"))
...
data_prep_ver4 %>%
  mutate(
    ...
    quarterly = lubridate::yq(yq))  -> data_prep_ver5 
...
data_prep_ver5 %>% 
  mutate(
    content = content %>% 
      ...
      str_replace_all("경향신문", "") %>% 
      str_replace_all("\\[.*\\]", "") %>% 
      str_replace_all("[a-zA-Z0-9]{1,}@.*", "") %>% 
      str_replace_all("[가-힣]{2,4} (기자|특파원)", "")
  ) -> data_prep_ver6

data_prep_ver6 %>% write_excel_csv("data_prep_ver6.csv")
